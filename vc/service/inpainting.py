...
class InpaintingService:
    ...
    def handle(self, args: InpaintingOptions):
        ...
        for idx in tqdm(range(len(sample_list))):
            ...
            mesh_fi = os.path.join(args.mesh_folder,
                                   sample['src_pair_name'] + '.ply')
            image = imageio.imread(sample['ref_img_fi'])

            print(f"Running depth extraction at {time.time()}")
            if args.use_boostmonodepth is True:
                run_boostmonodepth(sample['ref_img_fi'], args.src_folder,
                                   args.depth_folder)
            elif args.require_midas is True:
                run_depth([sample['ref_img_fi']], args.src_folder,
                          args.depth_folder,
                          args.MiDaS_model_ckpt, MonoDepthNet, MiDaS_utils,
                          target_w=640)
            ...
            if 'npy' in args.depth_format:
                ...
                args.output_h, args.output_w = imageio.imread(
                    sample['depth_fi']).shape[:2]
            frac = args.longer_side_len / max(args.output_h,
                                                   args.output_w)
            args.output_h, args.output_w = int(
                args.output_h * frac), int(args.output_w * frac)
            ...
            image = cv2.resize(image, (args.output_w, args.output_h),
                               interpolation=cv2.INTER_AREA)
            depth = read_MiDaS_depth(sample['depth_fi'], 3.0,
            ...
            if not (args.load_ply is True and os.path.exists(mesh_fi)):
                ...
                depth_edge_weight = torch.load(args.depth_edge_model_ckpt,
                ...
                depth_feat_weight = torch.load(args.depth_feat_model_ckpt,
                                               map_location=torch.device(
                                                   device))
                ...
                rgb_feat_weight = torch.load(args.rgb_feat_model_ckpt,
