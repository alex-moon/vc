...
def extrapolate(
    ...
    noext_H, noext_W = global_mesh.graph['noext_H'], global_mesh.graph[
        'noext_W']
    ...
    if "up" in direc.lower() and "-" not in direc.lower():
        ...
        context_anchor = [h_off, h_off + args.context_thickness, w_off,
                          w_off + noext_W]
        valid_line_anchor = [h_off, h_off + 1, w_off, w_off + noext_W]
        valid_anchor = [min(mask_anchor[0], context_anchor[0]),
                        max(mask_anchor[1], context_anchor[1]),
                        min(mask_anchor[2], context_anchor[2]),
                        max(mask_anchor[3], context_anchor[3])]
    elif "down" in direc.lower() and "-" not in direc.lower():
        all_anchor = [h_off + noext_H - args.context_thickness,
                      2 * h_off + noext_H, w_off, w_off + noext_W]
        global_shift = [all_anchor[0], all_anchor[2]]
        mask_anchor = [h_off + noext_H, 2 * h_off + noext_H, w_off,
                       w_off + noext_W]
        context_anchor = [h_off + noext_H - args.context_thickness,
                          h_off + noext_H, w_off, w_off + noext_W]
        valid_line_anchor = [h_off + noext_H - 1, h_off + noext_H, w_off,
                             w_off + noext_W]
        valid_anchor = [min(mask_anchor[0], context_anchor[0]),
                        max(mask_anchor[1], context_anchor[1]),
                        min(mask_anchor[2], context_anchor[2]),
        ...
        context_anchor = [h_off, h_off + noext_H, w_off,
                          w_off + args.context_thickness]
        ...
        all_anchor = [h_off, h_off + noext_H,
                      w_off + noext_W - args.context_thickness,
                      2 * w_off + noext_W]
        global_shift = [all_anchor[0], all_anchor[2]]
        mask_anchor = [h_off, h_off + noext_H, w_off + noext_W,
                       2 * w_off + noext_W]
        context_anchor = [h_off, h_off + noext_H,
                          w_off + noext_W - args.context_thickness,
                          w_off + noext_W]
        valid_line_anchor = [h_off, h_off + noext_H, w_off + noext_W - 1,
                             w_off + noext_W]
        valid_anchor = [min(mask_anchor[0], context_anchor[0]),
                        max(mask_anchor[1], context_anchor[1]),
                        min(mask_anchor[2], context_anchor[2]),
        ...
        all_anchor = [0, h_off + args.context_thickness, 0,
                      w_off + args.context_thickness]
        global_shift = [all_anchor[0], all_anchor[2]]
        ...
        all_anchor = [h_off + noext_H - args.context_thickness,
                      2 * h_off + noext_H, 0, w_off + args.context_thickness]
        global_shift = [all_anchor[0], all_anchor[2]]
        ...
        all_anchor = [0, h_off + args.context_thickness,
                      w_off + noext_W - args.context_thickness,
                      2 * w_off + noext_W]
        global_shift = [all_anchor[0], all_anchor[2]]
        ...
        all_anchor = [h_off + noext_H - args.context_thickness,
                      ...
                      2 * w_off + noext_W]
        global_shift = [all_anchor[0], all_anchor[2]]
        mask_anchor = [h_off + noext_H, 2 * h_off + noext_H, w_off + noext_W,
                       2 * w_off + noext_W]
        context_anchor = "inv-mask"
        valid_line_anchor = None
        valid_anchor = all_anchor
    ...
    global_mask[mask_anchor[0]:mask_anchor[1],
    mask_anchor[2]:mask_anchor[3]] = 1
    mask = global_mask[valid_anchor[0]:valid_anchor[1],
           valid_anchor[2]:valid_anchor[3]] * 1
    context = 1 - mask
    global_context = np.zeros_like(depth)
    global_context[all_anchor[0]:all_anchor[1],
    all_anchor[2]:all_anchor[3]] = context
    # context = global_context[valid_anchor[0]:valid_anchor[1], valid_anchor[2]:valid_anchor[3]] * 1

    valid_area = mask + context
    input_rgb = image[valid_anchor[0]:valid_anchor[1],
                valid_anchor[2]:valid_anchor[3]] / 255. * context[..., None]
    input_depth = depth[valid_anchor[0]:valid_anchor[1],
                  valid_anchor[2]:valid_anchor[3]] * context
    ...
    if valid_line_anchor is not None:
        valid_line[valid_line_anchor[0]:valid_line_anchor[1],
        valid_line_anchor[2]:valid_line_anchor[3]] = 1
    valid_line = valid_line[all_anchor[0]:all_anchor[1],
                 all_anchor[2]:all_anchor[3]]
    ...
    # pass
    # return
    input_edge_map = edge_map[all_anchor[0]:all_anchor[1],
                     all_anchor[2]:all_anchor[3]] * context
    input_other_edge_with_id = other_edge_with_id[all_anchor[0]:all_anchor[1],
                               all_anchor[2]:all_anchor[3]]
    ...
    valid_edge_ids = valid_edge_ids[1:] if (
        len(valid_edge_ids) > 0 and valid_edge_ids[
        0] == -1) else valid_edge_ids
    edge = reduce(
        lambda x, y: (
            x + (input_other_edge_with_id == y).astype(np.uint8)).clip(
            0,
            1
        ),
    ...
    t_rgb = torch.FloatTensor(input_rgb).to(device).permute(2, 0, 1).unsqueeze(
        0
    )
    ...
    t_depth_zero_mean_depth = \
        torch.FloatTensor(input_zero_mean_depth).to(device)[None, None, ...]

    depth_edge_output = depth_edge_model.forward_3P(
        t_mask, t_context, t_rgb, t_disp, t_edge, unit_length=128,
        cuda=device
    )
    t_output_edge = (
                        depth_edge_output > args.ext_edge_threshold).float() * t_mask + t_edge
    ...
    for hx, hy in zip(hxs, hys):
        ...
        eight_nes = [ne for ne in
                     [(hx + 1, hy), (hx - 1, hy), (hx, hy + 1), (hx, hy - 1), \
                      ...
                      (hx + 1, hy - 1)] \
                     if 0 <= ne[0] < output_raw_edge.shape[0] and 0 <= ne[1] <
                     output_raw_edge.shape[1] and 0 < output_raw_edge[
                         ne[0], ne[1]]]
        for ne in eight_nes:
    ...
    for end_pt, cc in zip(end_pts, ccs):
        ...
        if len(end_pt) == 1:
            ...
            ends = [*end_pt]
            edge_id = global_mesh.nodes[(
                ends[0][0] + all_anchor[0], ends[0][1] + all_anchor[2],
                -ends[0][2])]['edge_id']
            pnodes = sorted(
                pnodes,
            ...
            fpath = []
            if global_mesh.nodes[(
                ends[0][0] + all_anchor[0], ends[0][1] + all_anchor[2],
                -ends[0][2])].get('far') is None:
                print("None far")
                pass
            else:
                fnodes = global_mesh.nodes[(
                    ends[0][0] + all_anchor[0], ends[0][1] + all_anchor[2],
                    -ends[0][2])].get('far')
                fnodes = [(xx[0] - all_anchor[0], xx[1] - all_anchor[2], xx[2])
                          for xx in fnodes]
                dmask = mask + 0
                did = 0
                while True:
                ...
                for step in range(0, len(npath) - 1):
                    parr = (npath[step + 1][0] - npath[step][0],
                            npath[step + 1][1] - npath[step][1])
                    ...
                    new_loc_nes = [xx for xx in [(new_loc[0] + 1, new_loc[1]),
                                                 ...
                                                 (new_loc[0], new_loc[1] - 1)] \
                                   if
                                   xx[0] >= 0 and xx[0] < fpath_map.shape[0] and
                                   xx[1] >= 0 and xx[1] < fpath_map.shape[1]]
                    if np.sum(
                    ...
                    if valid_area[new_loc[0], new_loc[1]] == 0:
                        break
                    new_loc_nes_eight = [xx for xx in
                                         [(new_loc[0] + 1, new_loc[1]),
                                          (new_loc[0] - 1, new_loc[1]),
                                          (new_loc[0], new_loc[1] + 1),
                                          (new_loc[0], new_loc[1] - 1),
                                          (new_loc[0] + 1, new_loc[1] + 1),
                                          (new_loc[0] + 1, new_loc[1] - 1),
                                          (new_loc[0] - 1, new_loc[1] - 1),
                                          (new_loc[0] - 1, new_loc[1] + 1)] \
                                         if
                                         ...
                                         fpath_map.shape[1]]
                    if np.sum(
                        [int(npath_map[nlne[0], nlne[1]] == edge_id) for nlne in
                         new_loc_nes_eight]
                    ) == 0:
                        break
                    fpath.append(
                        (fpath[-1][0] + parr[0], fpath[-1][1] + parr[1])
                    )
                if step != len(npath) - 2:
    ...
    depth_output = np.exp(
        depth_output + input_mean_depth
    ) * mask  # + input_depth * context
    ...
    for near_id in np.unique(npath_map[npath_map > -1]):
        depth_output = refine_depth_around_edge(
            depth_output.copy(),
            (fpath_map == near_id).astype(np.uint8) * mask,
            # far_edge_map_in_mask,
    ...
    #     pass
    #     f, ((ax1, ax2)) = plt.subplots(1, 2, sharex=True, sharey=True); ax1.imshow(depth_output); ax2.imshow(npath_map + fpath_map); plt.show()
    rgb_output = rgb_feat_model.forward_3P(
        t_mask, t_context, t_rgb, t_update_edge, unit_length=128,
        cuda=device
    )
    ...
    rgb_output = ((rgb_output.squeeze().data.cpu().permute(
        1,
        2,
        0
    ).numpy() * mask[..., None] + input_rgb) * 255).astype(np.uint8)
    image[all_anchor[0]:all_anchor[1], all_anchor[2]:all_anchor[3]][mask > 0] = \
        rgb_output[mask > 0]  # np.array([255,0,0]) # rgb_output[mask > 0]
    depth[all_anchor[0]:all_anchor[1], all_anchor[2]:all_anchor[3]][mask > 0] = \
        depth_output[mask > 0]
    ...
    for nx, ny in zip(nxs, nys):
        ...
        four_nes = [xx for xx in
                    ...
                    if 0 <= xx[0] < fpath_map.shape[0] and 0 <= xx[1] <
                    fpath_map.shape[1]]
        for nex, ney in four_nes:
            if fpath_map[nex, ney] == n_id:
                na, nb = (nx + all_anchor[0], ny + all_anchor[2],
                          info_on_pix[(nx + all_anchor[0], ny + all_anchor[2])][
                              0]['depth']), \
                         (nex + all_anchor[0], ney + all_anchor[2], info_on_pix[
                             (nex + all_anchor[0], ney + all_anchor[2])][0][
                             'depth'])
    ...
    for nx, ny in zip(nxs, nys):
        ...
        four_nes = [xx for xx in
                    [(nx + 1, ny), (nx - 1, ny), (nx, ny + 1), (nx, ny - 1)] \
                    if 0 <= xx[0] < npath_map.shape[0] and 0 <= xx[1] <
                    npath_map.shape[1]]
        for nex, ney in four_nes:
    ...
    for x, y in zip(nxs, nys):
        ...
        info_on_pix, global_mesh = update_info(
            mapping_dict,
            info_on_pix,
            global_mesh
        )
    ...
    nxs, nys = np.where((npath_map > -1))
    for nx, ny in zip(nxs, nys):
        self_node = (nx + all_anchor[0], ny + all_anchor[2],
                     info_on_pix[(nx + all_anchor[0], ny + all_anchor[2])][0][
                         'depth'])
        ...
        four_nes = [xx for xx in
                    ...
                    fpath_map.shape[1]]
        for nex, ney in four_nes:
            ne_node = (nex + all_anchor[0], ney + all_anchor[2],
                       info_on_pix[(nex + all_anchor[0], ney + all_anchor[2])][
                           0]['depth'])
            ...
            if fpath_map[nex, ney] == n_id:
                if global_mesh.nodes[self_node].get('edge_id') is None:
                    ...
                    info_on_pix[(self_node[0], self_node[1])][0][
                        'edge_id'] = n_id
    ...
    global_fpath_map[all_anchor[0]:all_anchor[1],
    all_anchor[2]:all_anchor[3]] = fpath_map
    fpath_ids = np.unique(global_fpath_map)
    fpath_ids = fpath_ids[1:] if fpath_ids.shape[0] > 0 and fpath_ids[
        0] == -1 else []
    fpath_real_id_map = np.zeros_like(global_fpath_map) - 1
    for fpath_id in fpath_ids:
        fpath_real_id = np.unique(
            ((global_fpath_map == fpath_id).astype(np.int) * (
                other_edge_with_id + 1)) - 1
        )
        fpath_real_id = fpath_real_id[1:] if fpath_real_id.shape[0] > 0 and \
                                             fpath_real_id[0] == -1 else []
    ...
    for nx, ny in zip(nxs, nys):
        ...
        for nex, ney in four_nes:
            ...
            if global_mesh.has_node(ne_node) is False:
                continue
            if npath_map[nex, ney] == n_id or global_mesh.nodes[ne_node].get(
                'edge_id'
            ) == n_id:
                if global_mesh.has_edge(self_node, ne_node) is True:
                    global_mesh.remove_edge(self_node, ne_node)
                ...
                if global_mesh.nodes[self_node].get('edge_id') is None:
                    f_id = int(
                        round(fpath_real_id_map[self_node[0], self_node[1]])
                    )
...
def fill_dummy_bord(mesh, info_on_pix, image, depth, args):
    ...
    for x, y in zip(xs, ys):
        ...
        info_on_pix[(x, y)] = [{
            'depth': cur_depth,
            'color': mesh.nodes[(x, y, cur_depth)][
                'color'],
            'synthesis': False,
        ...
        # for x, y in zip(xs, ys):
        four_nes = [(xx, yy) for xx, yy in
                    [(x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1)] if \
                    0 <= x < mesh.graph['H'] and 0 <= y < mesh.graph[
                        'W'] and info_on_pix.get((xx, yy)) is not None]
        for ne in four_nes:
            # if (ne[0] - x) + (ne[1] - y) == 1 and info_on_pix.get((ne[0], ne[1])) is not None:
            mesh.add_edge(
                cur_node,
                (ne[0], ne[1], info_on_pix[(ne[0], ne[1])][0]['depth'])
            )
...
def enlarge_border(mesh, info_on_pix, depth, image, args):
    mesh.graph['hoffset'], mesh.graph[
        'woffset'] = args.extrapolation_thickness, args.extrapolation_thickness
    mesh.graph['bord_up'], mesh.graph['bord_left'], mesh.graph['bord_down'], \
    mesh.graph['bord_right'] = \
...
def refresh_bord_depth(mesh, info_on_pix, image, depth):
    ...
    # (0, W - 1), (H - 1, 0), (H - 1, W - 1)]
    bord_nodes = []
    bord_nodes += [(mesh.graph['bord_up'], xx) for xx in range(
        mesh.graph['bord_left'] + 1,
        mesh.graph['bord_right'] - 1
    )]
    bord_nodes += [(mesh.graph['bord_down'] - 1, xx) for xx in range(
        mesh.graph['bord_left'] + 1,
        mesh.graph['bord_right'] - 1
    )]
    bord_nodes += [(xx, mesh.graph['bord_left']) for xx in range(
        mesh.graph['bord_up'] + 1,
        mesh.graph['bord_down'] - 1
    )]
    bord_nodes += [(xx, mesh.graph['bord_right'] - 1) for xx in range(
        mesh.graph['bord_up'] + 1,
        mesh.graph['bord_down'] - 1
    )]
    for xy in bord_nodes:
        ...
        if tgt_loc is not None:
            ...
            tgt_nes_loc = [
                (xx[0] - tgt_loc[0] + xy[0], xx[1] - tgt_loc[1] + xy[1]) for xx
                in tgt_nes_loc \
                if abs(xx[0] - xy[0]) == 1 and abs(xx[1] - xy[1]) == 1]
            tgt_nes_loc = [xx for xx in tgt_nes_loc if
                           info_on_pix.get(xx) is not None]
            ...
            if info_on_pix.get(xy) is not None and len(info_on_pix.get(xy)) > 0:
                ...
                mesh.remove_edges_from(
                    [(old_ne, old_node) for old_ne in mesh.neighbors(old_node)]
                )
                mesh.add_edges_from(
                    [((zz[0], zz[1], info_on_pix[zz][0]['depth']), old_node) for
                     zz in tgt_nes_loc]
                )
                ...
                mesh.add_node(new_node)
                mesh.add_edges_from(
                    [((zz[0], zz[1], info_on_pix[zz][0]['depth']), new_node) for
                     zz in tgt_nes_loc]
                )
            mesh.nodes[new_node]['far'] = None
            mesh.nodes[new_node]['near'] = None
            if mesh.nodes[src_node].get('far') is not None:
                redundant_nodes = [ne for ne in mesh.nodes[src_node]['far'] if
                                   (ne[0], ne[1]) == xy]
                [mesh.nodes[src_node]['far'].remove(aa) for aa in
                 redundant_nodes]
            if mesh.nodes[src_node].get('near') is not None:
                redundant_nodes = [ne for ne in mesh.nodes[src_node]['near'] if
                                   (ne[0], ne[1]) == xy]
                [mesh.nodes[src_node]['near'].remove(aa) for aa in
                 redundant_nodes]
    for xy in corner_nodes:
        ...
        for ne_loc in four_nes:
            if info_on_pix.get(ne_loc) is not None:
                ...
                ne_nodes.append(
                    (ne_loc[0], ne_loc[1], info_on_pix[ne_loc][0]['depth'])
                )
    ...
    for xy in bord_nodes:
        ...
        four_nes = set(
            [(xy[0] + 1, xy[1]), (xy[0] - 1, xy[1]), (xy[0], xy[1] + 1),
             (xy[0], xy[1] - 1)]
        ) - \
                   set([(ne[0], ne[1]) for ne in nes])
        four_nes = [ne for ne in four_nes if
                    mesh.graph['bord_up'] <= ne[0] < mesh.graph['bord_down'] and \
                    mesh.graph['bord_left'] <= ne[1] < mesh.graph['bord_right']]
        four_nes = [(ne[0], ne[1], info_on_pix[(ne[0], ne[1])][0]['depth']) for
                    ne in four_nes]
...
def incomplete_node(mesh, edge_maps, info_on_pix):
    ...
    for node in mesh.nodes:
        if mesh.nodes[node].get('synthesis') is not True:
            ...
            nes = [xx for xx in mesh.neighbors(node) if
                   mesh.nodes[xx].get('synthesis') is not True]
            if len(nes) < 3 and 0 < node[0] < mesh.graph['H'] - 1 and 0 < node[
                1] < mesh.graph['W'] - 1:
                if len(nes) <= 1:
...
def edge_inpainting(
    ...
    edge_dict = get_edge_from_nodes(
        context_cc, erode_context_cc, mask_cc, edge_cc, extend_edge_cc,
    ...
    patch_edge_dict['output'] = tensor_edge_dict[
        'output'].squeeze().data.cpu().numpy()
    edge_dict['output'] = np.zeros((mesh.graph['H'], mesh.graph['W']))
    edge_dict['output'][union_size['x_min']:union_size['x_max'],
...
def depth_inpainting(
    ...
    depth_feat_model,
    edge_output,
    given_depth_dict=False,
    spdb=False
):
    if given_depth_dict is False:
        depth_dict = get_depth_from_nodes(
    ...
    patch_depth_dict['mask'], patch_depth_dict['context'], patch_depth_dict[
        'depth'], \
    ...
    tensor_depth_dict['output'] = torch.exp(
        depth_output + depth_dict['mean_depth']
    ) * \
                                  tensor_depth_dict['mask'] + tensor_depth_dict[
                                      'depth']
    patch_depth_dict['output'] = tensor_depth_dict[
        'output'].data.cpu().numpy().squeeze()
    depth_dict['output'] = np.zeros((mesh.graph['H'], mesh.graph['W']))
    depth_dict['output'][union_size['x_min']:union_size['x_max'],
    union_size['y_min']:union_size['y_max']] = \
        patch_depth_dict['output']
    depth_output = depth_dict['output'] * depth_dict['mask'] + depth_dict[
        'depth'] * depth_dict['context']
    depth_output = smooth_cntsyn_gap(
        depth_dict['output'].copy() * depth_dict['mask'] + depth_dict['depth'] *
        depth_dict['context'],
    ...
    if spdb is True:
        ...
        ax1.imshow(depth_output * depth_dict['mask'] + depth_dict['depth']);
        ax2.imshow(
            depth_dict['output'] * depth_dict['mask'] + depth_dict['depth']
        );
        plt.show()
        pass
    depth_dict['output'] = depth_output * depth_dict['mask'] + depth_dict[
        'depth'] * depth_dict['context']
...
def recursive_add_edge(edge_mesh, mesh, info_on_pix, cur_node, mark):
    ...
    for node_xy in ne_nodes:
        ...
        if mark[node[0], node[1]] != 3:
            ...
            info_on_pix, edge_mesh, mesh = update_info(
                mapping_dict,
                info_on_pix,
                edge_mesh,
                mesh
...
def resize_for_edge(tensor_dict, largest_size):
    ...
    if frac < 1:
        ...
        resize_dict['edge'] = torch.nn.functional.interpolate(
            resize_dict['edge'],
            scale_factor=frac,
            mode='bilinear'
        )
        ...
        resize_dict['disp'] = torch.nn.functional.interpolate(
            resize_dict['disp'],
            scale_factor=frac,
            mode='nearest'
        )
        resize_dict['disp'] = resize_dict['disp'] * resize_dict['context']
        resize_dict['rgb'] = torch.nn.functional.interpolate(
            resize_dict['rgb'],
            scale_factor=frac,
            mode='bilinear'
        )
...
def repaint_info(mesh, cc, x_anchor, y_anchor, source_type):
    if source_type == 'rgb':
        feat = np.zeros(
            (3, x_anchor[1] - x_anchor[0], y_anchor[1] - y_anchor[0])
        )
    else:
        feat = np.zeros(
            (1, x_anchor[1] - x_anchor[0], y_anchor[1] - y_anchor[0])
        )
    for node in cc:
        if source_type == 'rgb':
            feat[:, node[0] - x_anchor[0], node[1] - y_anchor[0]] = np.array(
                mesh.nodes[node]['color']
            ) / 255.
        elif source_type == 'd':
...
def get_depth_from_maps(
    ...
    rt_dict = {
        ...
        'zero_mean_depth': zero_mean_depth,
        'edge': edge
    }
...
def get_rgb_from_nodes(context_cc, erode_context_cc, mask_cc, H, W, mesh):
    ...
    for node in erode_context_cc:
        ...
        mask[node[0], node[1]] = 1
    rt_dict = {
        'rgb': rgb, 'context': context, 'mask': mask,
        'erode': erode_context
    }
