...
def create_mesh(depth, image, int_mtx, args):
    H, W, C = image.shape
    ext_H, ext_W = H + 2 * args.extrapolation_thickness, W + 2 * args.extrapolation_thickness
    LDI = netx.Graph(H=ext_H, W=ext_W, noext_H=H, noext_W=W, cam_param=int_mtx)
    xy2depth = {}
    int_mtx_pix = int_mtx * np.array([[W], [H], [1.]])
    LDI.graph['cam_param_pix'], LDI.graph[
        'cam_param_pix_inv'] = int_mtx_pix, np.linalg.inv(int_mtx_pix)
    disp = 1. / (-depth)
    LDI.graph['hoffset'], LDI.graph[
    ...
    LDI.graph['bord_up'], LDI.graph['bord_down'] = LDI.graph['hoffset'] + 0, \
                                                   LDI.graph['hoffset'] + H
    LDI.graph['bord_left'], LDI.graph['bord_right'] = LDI.graph['woffset'] + 0, \
                                                      LDI.graph['woffset'] + W
    for idx in range(H):
        for idy in range(W):
    ...
    for x, y, d in LDI.nodes:
        two_nes = [ne for ne in [(x + 1, y), (x, y + 1)] if
                   ne[0] < LDI.graph['bord_down'] and ne[1] < LDI.graph[
                       'bord_right']]
        [LDI.add_edge((ne[0], ne[1], xy2depth[ne][0]), (x, y, d)) for ne in
         two_nes]
...
def tear_edges(mesh, threshold=0.00025, xy2depth=None):
    remove_edge_list = []
    remove_horizon, remove_vertical = np.zeros(
        (2, mesh.graph['H'], mesh.graph['W'])
    )
    mesh_nodes = mesh.nodes
    for edge in mesh.edges:
        if abs(
            mesh_nodes[edge[0]]['disp'] - mesh_nodes[edge[1]]['disp']
        ) > threshold:
            remove_edge_list.append((edge[0], edge[1]))

            near, far = edge if abs(edge[0][2]) < abs(edge[1][2]) else edge[
                                                                       ::-1]

            mesh_nodes[far]['near'] = [] if mesh_nodes[far].get(
                'near'
            ) is None else mesh_nodes[far]['near'].append(near)
            mesh_nodes[near]['far'] = [] if mesh_nodes[near].get(
                'far'
            ) is None else mesh_nodes[near]['far'].append(far)
    ...
    dang_horizon = np.where(
        np.roll(remove_horizon, 1, 0) + np.roll(
            remove_horizon,
            -1,
            0
        ) - remove_horizon == 2
    ...
    dang_vertical = np.where(
        np.roll(remove_vertical, 1, 1) + np.roll(
            remove_vertical,
            -1,
            1
        ) - remove_vertical == 2
    ...
    horizon_condition = lambda x, y: mesh.graph['bord_up'] + 1 <= x < \
                                     mesh.graph['bord_down'] - 1
    vertical_condition = lambda x, y: mesh.graph['bord_left'] + 1 <= y < \
    ...
    for x, y in zip(dang_horizon[0], dang_horizon[1]):
        if horizon_condition(x, y) and node_existence(x, y) and node_existence(
            x,
            y + 1
        ):
            remove_edge_list.append((prjto3d(x, y), prjto3d(x, y + 1)))
    for x, y in zip(dang_vertical[0], dang_vertical[1]):
        if vertical_condition(x, y) and node_existence(x, y) and node_existence(
            x + 1,
            y
        ):
...
def reproject_3d_int_detail(
    ...
    abs_z = abs(z)
    return [abs_z * ((sy + 0.5 - w_offset) * k_00 + k_02),
            abs_z * ((sx + 0.5 - h_offset) * k_11 + k_12), abs_z]
...
def reproject_3d_int_detail_FB(sx, sy, z, w_offset, h_offset, mesh):
    ...
    ray = np.array(
        [(-1. + 2. * ((sy + 0.5 - w_offset) / (mesh.graph['W'] - 1))) *
         mesh.graph['tan_hFov'],
         (1. - 2. * (sx + 0.5 - h_offset) / (mesh.graph['H'] - 1)) * mesh.graph[
             'tan_vFov'],
         -1]
    )
...
def reproject_3d_int(sx, sy, z, mesh):
    ...
    ray = np.dot(
        k,
        np.array(
            [sy - mesh.graph['woffset'], sx - mesh.graph['hoffset'], 1]
        ).reshape(3, 1)
    )
...
def generate_init_node(mesh, args, min_node_in_cc):
    mesh_nodes = mesh.nodes
...
def generate_face(mesh, info_on_pix, args):
    ...
    for x in range(H):
        for y in range(W):
            if is_inside(
                ...
                bord_left,
                bord_right
            ) and not (key_exist(info_on_pix, (x, y))):
                lost_map[x, y] = 1
    _, label_lost_map = cv2.connectedComponents(
        lost_map.astype(np.uint8),
        connectivity=4
    )
    mask = np.zeros((H, W))
    ...
    for node in LDI.nodes:
        ...
        if len(neighbors) < 4:
            ...
            inval_diag_candi = set(
                [inval_diagonal for ne_node in discont_nes for inval_diagonal in
                 LDI.neighbors(ne_node) if \
                 abs(inval_diagonal[0] - node[0]) < 2 and abs(
                     inval_diagonal[1] - node[1]
                 ) < 2]
            )
            for ne_node in diag_candi_anc:
                if ne_node[0] == node[0]:
                    ...
                    diagonal_xys = [[ne_node[0], ne_node[1] + 1],
                                    [ne_node[0], ne_node[1] - 1]]
                for diag_candi in LDI.neighbors(ne_node):
                    if [diag_candi[0],
                        diag_candi[1]] in diagonal_xys and LDI.degree(
                        diag_candi
                    ) < 4:
    ...
    if remove_conflict_ordinal:
        ...
        for edge_id, discont_cc in enumerate(discont_ccs):
            ...
            for discont_node in discont_cc:
                near_flag = True if key_exist(
                    LDI.nodes[discont_node],
                    'far'
                ) else near_flag
                far_flag = True if key_exist(
                    LDI.nodes[discont_node],
                    'near'
                ) else far_flag
            ...
            if far_flag and near_flag:
                ...
                for discont_node in discont_cc:
                    ...
                    near_side = discont_graph.nodes[discont_node][
                                    'ordinal'] <= 0
                    if abs(ordinal_relation) < len(
                        ...
                        for ne_node in discont_graph.neighbors(discont_node):
                            remove_flag = (near_side and not (
                                key_exist(LDI.nodes[ne_node], 'far'))) or \
                                          (not near_side and not (key_exist(
                                              LDI.nodes[ne_node],
                                              'near'
                                          )))
                            remove_edges += [
                ...
                is_redun_near = lambda xx: len(xx) == 1 and xx[
                    0] in remove_nodes and key_exist(LDI.nodes[xx[0]], 'far')
            ...
            for ne_node in mesh.neighbors(valid_edge_node):
                if mesh_nodes[ne_node].get('far') is not None:
                    for fn in mesh_nodes[ne_node].get('far'):
                        if mesh.has_node(fn) and mesh_nodes[fn].get(
                            'edge_id'
                        ) is not None:
    ...
    for valid_edge_id, valid_edge_cc in enumerate(edge_ccs):
        for valid_edge_node in valid_edge_cc:
            ...
            if len([*edge_mesh.neighbors(valid_edge_node)]) == 1:
                num_ne = 1
                if num_ne == 1:
                    end_maps[valid_edge_node[0], valid_edge_node[1]] = \
                        valid_edge_node[2]
            ...
            if len([*edge_mesh.neighbors(valid_edge_node)]) == 3:
                ...
                if invalid is True:
                    ...
                    eight_nes = [(x, y) for x, y in
                                 ...
                                 edge_canvas[x, y] != -1 and edge_canvas[
                                     x, y] != valid_edge_id]
                    db_eight_nes = [(x, y) for x in range(hx - 2, hx + 3) for y
                                    in range(hy - 2, hy + 3) \
                                    if info_on_pix.get((x, y)) is not None and
                                    edge_canvas[x, y] != -1 and edge_canvas[
                                        x, y] != valid_edge_id and (x, y) != (
                                        hx, hy)]
...
def remove_dangling(mesh, edge_ccs, edge_mesh, info_on_pix, image, depth, args):
    ...
    for edge_cc_id, valid_edge_cc in enumerate(tmp_edge_ccs):
        ...
        eight_nes = set(
            [(x, y, info_on_pix[(x, y)][0]['depth']) for x, y in
        ...
        largest_cc = sorted(
            four_ccs,
            key=lambda x: (len(x), -np.sum([abs(xx[2] - hz) for xx in x]))
        )[-1]
        if len(largest_cc) < 2:
            for ne in four_nes:
            ...
            mesh.remove_edges_from(
                [(single_edge_node, ne) for ne in
                 mesh.neighbors(single_edge_node)]
            )
    ...
    for node in conn_1_nodes:
        ...
        self_nes = set(
            [ne2 for ne1 in mesh.neighbors(node) for ne2 in mesh.neighbors(ne1)
             if ne2 in eight_nes]
        )
        ...
        largest_cc = sorted(
            ccs,
            key=lambda x: (len(x), -np.sum(
                [abs(xx[0] - node[0]) + abs(xx[1] - node[1]) for xx in x]
            ))
        )[-1]
    ...
    for cc in ccs:
        ...
        for node in candidate_nodes:
            ...
            eight_nes = set(
                [(x, y, info_on_pix[(x, y)][0]['depth']) for x, y in
                 [(hx + 1, hy), (hx - 1, hy), (hx, hy + 1), (hx, hy - 1),
                  ...
                  (hx + 1, hy - 1)] \
                 if info_on_pix.get((x, y)) is not None and (
                     x, y, info_on_pix[(x, y)][0]['depth']) not in cc]
            )
            ...
            try:
                ...
                pass
            largest_cc = [xx for xx in ne_cc if
                          abs(xx[0] - node[0]) + abs(xx[1] - node[1]) == 1]
            mesh.remove_edges_from([(xx, node) for xx in mesh.neighbors(node)])
            re_depth = {'value': 0, 'count': 0}
            for cc_node in largest_cc:
                re_depth['value'] += cc_node[2]
        ...
        if len(cc) == 1:
            ...
            nine_nes = set(
                [(x, y, info_on_pix[(x, y)][0]['depth']) for x, y in
                 [(hx, hy), (hx + 1, hy), (hx - 1, hy), (hx, hy + 1),
                  (hx, hy - 1),
                  (hx + 1, hy + 1), (hx - 1, hy - 1), (hx - 1, hy + 1),
                  (hx + 1, hy - 1)] \
                 if info_on_pix.get((x, y)) is not None and mesh.has_node(
                    (x, y, info_on_pix[(x, y)][0]['depth'])
                )]
            )
...
def context_and_holes(
    mesh, edge_ccs, args, specific_edge_id, specific_edge_loc, depth_feat_model,
    ...
    if inpaint_iter == 0:
        ...
        background_thickness = args.background_thickness_2
        context_thickness = args.context_thickness_2
    ...
    for edge_id, edge_cc in enumerate(edge_ccs):
        ...
        for edge_node in edge_cc:
            raw_intouched_nodes = set(mesh_nodes[edge_node].get('near')) if \
                mesh_nodes[edge_node].get('near') is not None else set()
            tmp_intouched_nodes |= set(
                [xx for xx in raw_intouched_nodes if
                 mesh_nodes[xx].get('edge_id') is not None and \
    ...
    for edge_id, edge_cc in enumerate(edge_ccs):
            ...
            if i == 0:
                ...
                for node in tmp_mask_nodes:
                    ...
                    if mesh_nodes[node].get('connect_point_id') is not None:
                        connect_node.append(
                            mesh_nodes[node]['connect_point_id']
                        )
                    connect_point_id = np.bincount(
                        connect_node
                    ).argmax() if len(connect_node) > 0 else -1
                    ...
                    if mesh_nodes[node].get(
                        'connect_point_exception'
                    ) is not None:
                ...
                if (context_map.astype(np.uint8) * mask_map.astype(
                    np.uint8
                )).max() > 0:
                    pass
            ...
            for node in tmp_mask_nodes:
                four_nes = {xx: [] for xx in
                            [(node[0] - 1, node[1]), (node[0] + 1, node[1]),
                             (node[0], node[1] - 1), (node[0], node[1] + 1)] if \
                            0 <= xx[0] < connect_map.shape[0] and 0 <= xx[1] <
                            connect_map.shape[1]}
                if inpaint_iter > 0:
                ...
                for ne in nes:
                    if bool(context_map[ne[0], ne[1]]) is False and \
                        ...
                        if i < background_thickness:
                            ...
                            if inpaint_iter == 0:
                                if comp_cnt_depth[node[0], node[1]] > 0 and \
                                    comp_cnt_depth[ne[0], ne[1]] == 0:
                                    comp_cnt_depth[ne[0], ne[1]] = \
                                ...
                                if mesh_nodes[ne].get('edge_id') is not None and \
                                    ...
                                    for intouched_far in accum_intouched_fars:
                                        if bool(
                                            mask_map[
                                                intouched_far[0], intouched_far[
                                                    1]]
                                        ) is True or \
                                            bool(
                                                context_map[intouched_far[0],
                                                            intouched_far[1]]
                                            ) is True:
                                            continue
                                ...
                                if mesh_nodes[ne].get('near') is not None:
                                    intouched_nears = set(
                                        mesh_nodes[ne].get('near')
                                    )
                                    for intouched_near in intouched_nears:
                                        if bool(
                                            mask_map[intouched_near[0],
                                                     intouched_near[1]]
                                        ) is True or \
                                            bool(
                                                context_map[intouched_near[0],
                                                            intouched_near[1]]
                                            ) is True:
                                            continue
                                        tmp_redundant_nodes.add(intouched_near)
                                        intouched_map[
                                            intouched_near[0], intouched_near[
                                                1]] = True
                        if not (mesh_nodes[ne].get(
                            'inpaint_id'
                        ) != 1 and inpaint_iter == 1):
                    ...
                    if bool(
                        ...
                        if i == passive_context and inpaint_iter == 1:
                            ...
                            if any(
                                ...
                                continue
                        if False and mesh_nodes[ne].get('near') is not None and \
                            mesh_nodes[ne].get('edge_id') != edge_id:
                            noncont_nears = set(mesh_nodes[ne].get('near'))
                            for noncont_near in noncont_nears:
                                if bool(
                                    context_map[
                                        noncont_near[0], noncont_near[1]]
                                ) is False:
                                    tmp_noncont_nodes.add(noncont_near)
                                    noncont_map[
                                        noncont_near[0], noncont_near[1]] = True
        ...
        accomp_extend_context_ccs[edge_id] |= set(
            cur_accomp_near_cc
        ).intersection(cur_mask_cc)
        extend_edge_ccs[edge_id] |= set(cur_accomp_near_cc).intersection(
            cur_mask_cc
        )
        ...
        if inpaint_iter == 0:
            tmp_width = args.depth_edge_dilate
        else:
            tmp_width = 0
        ...
        extend_context_ccs[edge_id] = extend_context_ccs[edge_id] - mask_ccs[
            edge_id] - accomp_extend_context_ccs[edge_id]
    if inpaint_iter == 0:
        all_ecnt_cc = set()
        for ecnt_id, ecnt_cc in enumerate(extend_context_ccs):
            ...
            for i in range(background_thickness):
                if i == 0:
                    ...
                    tmp_mask_nodes = copy.deepcopy(
                        accomp_extend_context_ccs[ecnt_id]
                    )
                    tmp_context_map = np.zeros(
                        (mesh.graph['H'], mesh.graph['W'])
                    ).astype(np.bool)
                    tmp_mask_map = np.zeros(
                        (mesh.graph['H'], mesh.graph['W'])
                    ).astype(np.bool)
            ...
            extend_context_ccs[ecnt_id] = extend_context_ccs[ecnt_id] - \
                                          mask_ccs[ecnt_id]
            extend_erode_context_ccs[ecnt_id] = extend_context_ccs[
                                                    ecnt_id] & constraint_erode_context_cc
            extend_context_ccs[ecnt_id] = extend_context_ccs[ecnt_id] - \
                                          extend_erode_context_ccs[ecnt_id] - \
                                          erode_context_ccs[ecnt_id]
            tmp_context_cc = context_ccs[ecnt_id] - extend_erode_context_ccs[
                ecnt_id] - erode_context_ccs[ecnt_id]
            if len(tmp_context_cc) > 0:
                context_ccs[ecnt_id] = tmp_context_cc
            tmp_mask_cc = tmp_mask_cc - context_ccs[ecnt_id] - \
                          erode_context_ccs[ecnt_id]
...
def DL_inpaint_edge(
    ...
    for edge_id, (context_cc, mask_cc, erode_context_cc, extend_context_cc,
        ...
        patch_edge_dict['mask'], patch_edge_dict['context'], patch_edge_dict[
            'rgb'], \
        ...
        if require_depth_edge(
            ...
            if ((depth_edge_output > args.ext_edge_threshold).float() *
                ...
                for nid in npaths.keys():
                    ...
                    if start_mx == -1:
                        pass
                    valid_end_pt = () if end_mx == -1 else (
                        end_mx, end_my,
                        info_on_pix[(end_mx, end_my)][0]['depth'])
                    new_edge_info = dict(
                        fpath=fpath,
                        npath=npath,
                    ...
                    if len(valid_end_pt) > 0:
                        new_edge_info = dict(
                            fpath=fpath[::-1],
                            npath=npath[::-1],
                            cont_end_pts=(start_mx, start_my,
                                          info_on_pix[(start_mx, start_my)][0][
                                              'depth']),
                            mask_id=edge_id,
                            comp_edge_id=nid,
    ...
    for edge_id, (context_cc, mask_cc, erode_context_cc, extend_context_cc,
            ...
            for end_pt in end_pts:
                ...
                if len(cur_info['cont_end_pts']) == 0 and len(other_infos) > 0:
                    other_infos = sorted(
                        other_infos,
                        key=lambda aa: abs(
                            abs(aa['cont_end_pts'][2]) - abs(cur_depth)
                        )
                    )
                    for other_info in other_infos:
                        tmp_fmap, tmp_nmap = np.zeros(
                            (mesh.graph['H'], mesh.graph['W'])
                        ) - 1, np.zeros(
                            (mesh.graph['H'], mesh.graph['W'])
                        ) - 1
                    ...
                    if min(tmp_fmap.max(), tmp_nmap.max()) != -1:
                        ...
                        edge_dict['fpath_map'][
                            edge_dict['valid_area'] == 0] = -1
                        edge_dict['npath_map'] = tmp_nmap
                        edge_dict['npath_map'][
                            edge_dict['valid_area'] == 0] = -1
                        discard_map = ((tmp_nmap != -1).astype(
                            np.uint8
                        ) + (tmp_fmap != -1).astype(np.uint8)) * edge_dict[
                                          'mask']
                    else:
                        for fnode in cur_info['fpath']:
        ...
        if frac < 1:
            ...
            resize_rgb_dict['rgb'] = resize_rgb_dict['rgb'] * resize_rgb_dict[
                'context']
        ...
        rgb_input_feat = torch.cat(
            (resize_rgb_dict['rgb'], resize_rgb_dict['edge']),
            dim=1
        )
        ...
        with torch.no_grad():
            ...
            rgb_output = rgb_output.cpu()
        resize_rgb_dict['output'] = rgb_output * resize_rgb_dict['mask'] + \
                                    resize_rgb_dict['rgb']
        ...
        if frac < 1:
            ...
            tensor_rgb_dict['output'] = tensor_rgb_dict['output'] * \
                                        tensor_rgb_dict['mask'] + (
                                            tensor_rgb_dict['rgb'] *
                                            tensor_rgb_dict['context'])
        patch_rgb_dict['output'] = tensor_rgb_dict[
            'output'].data.cpu().numpy().squeeze().transpose(1, 2, 0)
        rgb_dict['output'] = np.zeros((mesh.graph['H'], mesh.graph['W'], 3))
        rgb_dict['output'][union_size['x_min']:union_size['x_max'],
        union_size['y_min']:union_size['y_max']] = \
            ...
            if rgb_dict['mask'][node[0], node[1]] > 0:
                for info in info_on_pix[(node[0], node[1])]:
                    if abs(info['depth']) == abs(node[2]):
                        info['update_color'] = (rgb_dict['output'][
                                                    node[0], node[
                                                        1]] * 255).astype(
                            np.uint8
                        )
            ...
            if info_on_pix.get((node[0], node[1])) is not None:
                for info in info_on_pix.get((node[0], node[1])):
                    if info.get('inpaint_id') is None or abs(
                        ...
                        pre_depth = info['depth'] if info.get(
                            'real_depth'
                        ) is None else info['real_depth']
        ...
        for erode_node in erode_context_cc:
            for info in info_on_pix[(erode_node[0], erode_node[1])]:
                if info['depth'] == erode_node[2]:
                    ...
                    np_image[(erode_node[0], erode_node[1])] = info[
                        'update_color']
    ...
    for node in mesh.nodes:
        if len(node) == 2:
            ...
            continue
        if mesh.nodes[node].get('edge_id') is not None and mesh.nodes[node].get(
            'inpaint_id'
        ) == inpaint_iter + 1:
...
def write_ply(
    ...
    input_mesh, xy2depth, image, depth = create_mesh(
        depth,
        image,
        int_mtx,
        args
    ...
    input_mesh = tear_edges(input_mesh, args.depth_threshold, xy2depth)
    input_mesh, info_on_pix = generate_init_node(
        input_mesh,
        args,
        min_node_in_cc=200
    )
    ...
    mesh, info_on_pix, depth = fill_missing_node(
        input_mesh,
        info_on_pix,
        image,
        depth
    ...
    if args.extrapolate_border is True:
        ...
        edge_ccs, input_mesh, edge_mesh = \
            group_edges(input_mesh, args, image, remove_conflict_ordinal=True)
    ...
    vis_edge_ccs = filter_edge(input_mesh, edge_ccs, args)
    ...
    erode_context_ccs_canvas = np.zeros(
    ...
    vertex_id = 0
    input_mesh.graph['H'], input_mesh.graph['W'] = input_mesh.graph['noext_H'], \
                                                   input_mesh.graph['noext_W']
    background_canvas = np.zeros(
        (input_mesh.graph['H'],
         input_mesh.graph['W'],
         3)
    )
    ...
    for pix_xy, pix_list in info_on_pix.items():
        for pix_idx, pix_info in enumerate(pix_list):
            pix_depth = pix_info['depth'] if pix_info.get(
                'real_depth'
            ) is None else pix_info['real_depth']
            str_pt = [out_fmt(x, ply_flag) for x in reproject_3d_int_detail(
                pix_xy[0], pix_xy[1], pix_depth,
                k_00, k_02, k_11, k_12, w_offset, h_offset
            )]
            if input_mesh.has_node(
                (pix_xy[0], pix_xy[1], pix_info['depth'])
            ) is False:
                return False
                continue
            if pix_info.get('overlap_number') is not None:
                str_color = [out_fmt(x, ply_flag) for x in (
                    pix_info['color'] / pix_info['overlap_number']).astype(
                    np.uint8
                ).tolist()]
            else:
                str_color = [out_fmt(x, ply_flag) for x in
    ...
    if args.save_ply is True:
        ...
        with open(ply_name, 'w') as ply_fi:
            ...
            ply_fi.write(
                'comment hFov ' + str(float(input_mesh.graph['hFov'])) + '\n'
            )
            ply_fi.write(
                'comment vFov ' + str(float(input_mesh.graph['vFov'])) + '\n'
            )
